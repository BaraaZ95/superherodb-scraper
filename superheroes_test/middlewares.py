...
class CustomRetryMiddleware(RetryMiddleware):
    ...
    chrome_prefs["profile.managed_default_content_settings"] = {
        "images": 2}
    driver = uc.Chrome(
        options=options, driver_executable_path=path)
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200:  # If the response fails for any reason, render the page with Selenium
...
class SuperheroesTestSpiderMiddleware:
...
class SuperheroesTestDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
